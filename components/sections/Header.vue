            ...
            <nav class="container flex justify-between items-center p-2 text-white">
                ...
                <div v-if="nav.active_menu" class="lg:hidden">
                    ...
                    <div v-if="isOpen" class="absolute top-20 right-0 bg-black p-4 pb-40 shadow-md">
                        <div v-for="(link, index) in nav.menu.items" :key="index"
                            class="hover:text-primary duration-10 text-base mb-2">
                        ...
                        <div>
                            <ul v-for="(item, i) in otherMenu" :key="i" class="flex flex-row">
                                <li class="flex flex-row text items-center mb-1 justify-between w-full rounded-md ">
                                    <a class="p-2 m-1 w-full flex hover:text-primary" :href="item.url">{{ item.text }}</a>
                                ...
                                <transition name="slide">
                                    <div v-if="item._id == activeId">
                                        <div v-for="(item, i) in item.childrens" :key="i" class="">
                                            <a class="p-1 mx-1 rounded-md lg:bg-gray-50 text-white text-base mb-2 hover:transparent hover:text-primary duration-10 flex"
                                            ...
                                            <ul class="p-2" v-if="item.childrens && item.childrens.length > 0">
                                                <li v-for="(child, ii) in item.childrens" :key="ii">
                                                    <a class="p-1 mx-1 rounded-md lg:bg-gray-50 text-black text-base mb-2 hover:transparent hover:text-primary duration-10 flex"
                                                        :href="child.url">
                                                        {{ child.text }}
                                                    </a>
                                                </li>
                ...
                <!-- <div v-if="nav.active_menu" class="lg:hidden">
                    ...
                    <button @click="toggleMenu" class="text-white p-4" style="z-index: 1000; margin-right: 20px;">
                        <img class="h-8" src="~/assets/images/icon_menu.png" alt="toggleMenu" />
                    </button>
                            ...
                            <div class="fixed inset-y-0 right-0 w-64 bg-black p-4 pb-40 shadow-md">
                                ...
                                <div>
                                    <ul v-for="(item, i) in otherMenu" :key="i" class="flex flex-col">
                                        <li class="flex flex-row text items-center mb-1 justify-between w-full rounded-md ">
                                            <a class="p-2 m-1 w-full flex hover:text-primary" :href="item.url">
                                                {{ item.text }}
                                            </a>
                                        ...
                                        <transition name="slide">
                                            <div v-if="item._id == activeId">
                                                <div v-for="(item, i) in item.childrens" :key="i" class="pr-10 bg-gray-50 rounded-md">
                                                    <a class="p-1 mx-1 text-gray-400 rounded-md text-base mb-2 hover:bg-gray-50 hover:text-primary duration-10 flex"
                                                        :href="item.url">{{ item.text }}</a>
                                                    <ul class="p-2" v-if="item.childrens && item.childrens.length > 0">
                                                        <li v-for="(child, ii) in item.childrens" :key="ii">
                                                            <a class="p-1 mx-1 text-gray-400 rounded-md text-base mb-2 hover:bg-gray-50 hover:text-primary duration-10 flex"
                ...
                <div v-if="nav.active_menu" class="hidden lg:flex flex-row justify-center items-center  relative">
                    <div v-for="(link, index) in nav.menu.items" :key="index" class="text-xs mx-3 my-6">
                        <nuxt-link :to="link.url"
                            class=" hover:text-primary focus:underline focus:text-primary transition duration-1000 ease-in-out ">
                            {{ link.text }}
                        </nuxt-link>
                    </div>
                    <div class="flex flex-row ">
                        <ul v-for="(item, i) in otherMenu" :key="i" class="flex flex-col relative px-2"
                            ...
                            <li class="flex flex-row text items-center mb-1 justify-between w-full rounded-md "
                                ...
                                <a class=" w-full flex cursor-pointer hover:text-primary focus:underline focus:text-primary transition duration-1000 ease-in-out text-xs"
                                    :href="item.url">{{ item.text }}</a>
                            ...
                            <transition name="slide">
                                <div v-if="item.isClicked || item._id == activeId"
                                    ...
                                    <div v-for="(item, i) in item.childrens" :key="i" class="">
                                        <a class="flex p-3 mx-1 rounded-md  text-black text-xs mb-2 focus:underline focus:text-primary hover:text-primary duration-10 hover:bg-gray-200 "
                                            :href="item.url">{{ item.text }}</a>
                                        <ul class="p-2" v-if="item.childrens && item.childrens.length > 0">
                                            <li v-for="(child, ii) in item.childrens" :key="ii">
                                                <a class="p-1 mx-1 rounded-md  text-black text-xs mb-2 hover:transparent hover:text-primary duration-10 flex"
                                                    :href="child.url">
                                                    {{ child.text }}
                                                </a>
                                            </li>
