...
import { m as mergedTypes, c as getTypeDescriptor, n as normalizeTypes, f as cachedFn } from '../shared/untyped.bd7ca8a5.mjs';
import 'scule';
      ...
      VariableDeclaration(p) {
        ...
        if (t.isIdentifier(declaration.id) && (t.isFunctionExpression(declaration.init) || t.isArrowFunctionExpression(declaration.init))) {
          const newDeclaration = t.functionDeclaration(
            ...
            t.isBlockStatement(declaration.init.body) ? declaration.init.body : t.blockStatement([t.returnStatement(declaration.init.body)])
          );
...
function astify(val) {
  ...
  return t.objectExpression(
    Object.getOwnPropertyNames(val).filter((key) => val[key] !== void 0 && val[key] !== null).map((key) => t.objectProperty(t.identifier(key), astify(val[key])))
  );
}
